...
YOUR_DOMAIN = "https://alie-1.onrender.com" # ⚠️ دامنه شما برای Callback

PRICES = {
    'weekly': 250000,    # ۲۵ هزار تومان (به ریال)
    'monthly': 700000,   # ۷۰ هزار تومان (به ریال)
...
class UserUsage(db.Model):
    ...
    # 🔴 حذف 'package_chat_budget' از اینجا
...
def get_user_by_identifier(identifier):
    return User.query.filter(
        or_(User.email == identifier, User.phone == identifier, User.id == identifier)
    ).first()
...
def check_and_deduct_score(user_identifier, usage_type):
    ...
    # 🔴 این بخش احتمالاً باعث خطا می‌شد اگر 'package_chat_budget' در جایی دیگر فراخوانی می‌شد.
    # اگر 'package_chat_budget' در واقع همان 'extra_chat_packages' کاربر است،
    # باید منطق متفاوتی اعمال شود که خارج از این تابع پایه است.
...
def translate_prompt_to_english(persian_prompt):
    translation_system_prompt = (
        "You are an expert prompt engineer. Translate the following Persian description into a detailed, "
...
def user_action():
    ...
    return jsonify({
        "status": "success",
        "message": message,
        "new_status": {'is_premium': user.is_premium, 'is_banned': user.is_banned, 'score': user.score}
    })
...
def chat():
    ...
    is_active_premium = user and user.is_premium and user.premium_expiry and user.premium_expiry > datetime.utcnow()
    ...
    session["conversation"] = [] # پاک کردن تاریخچه برای محدودیت توکن
...
def image_generator():
    ...
    if user.is_banned:
        return jsonify({"status": "error", "message": "⛔ متأسفم، حساب کاربری شما توسط مدیر سیستم مسدود شده است."}), 403
...
def get_conversations_list():
    ...
    for chat in conversations_query:
        ...
        formatted_list.append({
            'id': chat.id, 'title': chat.title, 'last_update': date_str, 'preview': preview
        })
...
def bazaar_callback():
    ...
    try:
        ...
        return "خطا در احراز هویت با بازار. لطفاً دوباره تلاش کنید.", 500
    except Exception as e:
        print(f"General Bazaar Callback Error: {e}")
        return "خطای داخلی سرور در پردازش بازگشت از بازار.", 500
...
def initiate_pay(plan_type):
    ...
    amount = PRICES.get(plan_type, 30000)
    
    # 🔴 اصلاح دامنه Callback بر اساس آنچه در تعریف پروژه شما آمده (alie-1.onrender.com)
...
def migrate_database():
    with app.app_context():
        try:
            db.create_all()
            # اطمینان از وجود ستون‌های جدید در جداول
            
            # 1. اطمینان از وجود ستون‌های جدید در جدول 'users'
            ...
            # 2. اطمینان از وجود ستون‌های موجود در جدول 'user_usage'
            # (اگرچه در مدل جدید تعریف شده، این کار برای اطمینان از وجود ستون‌های قدیمی/جدید مفید است)
            db.session.execute(text('ALTER TABLE "user_usage" ADD COLUMN IF NOT EXISTS long_response_budget INTEGER DEFAULT 5'))
            db.session.execute(text('ALTER TABLE "user_usage" ADD COLUMN IF NOT EXISTS level_check VARCHAR(10)'))
            
            # 🔴 رفع مشکل ریشه‌ای: اگر ستون 'package_chat_budget' به اشتباه در جایی دیگر تعریف شده،
            # برای جلوگیری از خطای 'UndefinedColumn' در کوئری‌های آینده، آن را به صورت ایمن اضافه می‌کنیم.
            # اگر این ستون نباید وجود داشته باشد، بهتر است از ابزارهای Migration استفاده شود تا حذف شود.
            # در اینجا فقط آن را اضافه می‌کنیم تا کوئری‌ها شکست نخورند.
            db.session.execute(text('ALTER TABLE "user_usage" ADD COLUMN IF NOT EXISTS package_chat_budget INTEGER DEFAULT 0'))
