...
- پاسخ‌ها باید **فوق‌العاده کامل، مفصل و دقیق** باشند و در سقف نهایی **۸۰۰۰ توکن** به پایان برسند. (به هیچ عنوان پاسخ را از وسط جمله قطع نکن).
"""
# 💡 ثابت‌های جدید برای حالت پاسخ بلند (تنظیم برای حالت نامحدود)
LONG_RESPONSE_TOKEN_THRESHOLD = 0 # آستانه توکن ورودی برای پاسخ بلند (۰ یعنی همیشه فعال است)
LONG_RESPONSE_MAX_COMPLETION_TOKENS = 8000 # حداکثر توکن خروجی برای پاسخ بلند
LONG_RESPONSE_TOTAL_TOKEN_LIMIT = 16000 # سقف کل توکن (ورودی + خروجی) برای پاسخ بلند
...
def chat():
    ...
    # 💡 اجرای دستور کاربر: هیچ کاربری نتونه چت کنه اگر تو اکانت نبود.
    if not user or not user_identifier:
        return jsonify({"reply": "⛔ برای ادامه گفتگو، لطفاً ابتدا وارد حساب کاربری خود شوید."}), 403
    ...
    # 💡 منطق پاسخ بلند حذف شد. همه چت‌ها با سقف بالا اجرا می‌شوند.
    usage_type = 'chat' # همیشه هزینه چت عادی کسر می‌شود (1 امتیاز)
    ...
    # ما هزینه 'chat' را 1 امتیاز گذاشتیم و سقف آن را 8000 توکن.
    is_allowed, result = check_and_deduct_score(user_identifier, usage_type)
    if not is_allowed:
        return jsonify({"reply": result})
        
    # 💡 مدیریت کاربران مهمان حذف شد.
    ...
    if user and session.get('user_id'):
        ...
        # این بلوک برای مهمان بود که اکنون به دلیل چک لاگین بالا، اجرا نخواهد شد.
        session.pop('current_chat_id', None)
        if "conversation" not in session:
            session["conversation"] = []

    
    # 💡 تنظیم سقف توکن و System Prompt روی مقادیر بالای پیش‌فرض 
    # (حالت پاسخ بلند و عادی با هم ادغام شدند و سقف افزایش یافت)
    ...
    if remaining_tokens <= 500: # آستانه هشدار برای سقف 16000
        # اگر توکن کم است، هشدار بده
